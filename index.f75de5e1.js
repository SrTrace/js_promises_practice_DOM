...
}).then((button)=>new Promise((resolve, reject)=>{
        const message = "Third promise was resolved";
        if (button === 2) document.body.addEventListener("click", ()=>resolve(message));
        if (button === 0) document.body.addEventListener("contextmenu", (e)=>{
            e.preventDefault();
            resolve(message);
        });
    }));
firstPromise.then((result)=>handlerSuccess(result)).catch((error)=>handlerError(error.message));
